...
namespace Unity.Entities
{
    ...
    internal unsafe struct BufferHeader
    {
        ...
        {
            if (count <= header->Capacity && (count == 0 || header->Pointer != null))
                return;
        ...
        public static void SetCapacity(BufferHeader* header, int count, int typeSize, int alignment, TrashMode trashMode, bool useMemoryInitPattern, byte memoryInitPattern, int _)
        {
            ...
            byte* newData = (newCapacity <= 0) ? (byte*)(header + 1) : (byte*)Memory.Unmanaged.Allocate(newSizeInBytes, alignment, Allocator.Persistent);
...
#endif
                if (trashMode == TrashMode.RetainOldData && oldData != null)
                {
