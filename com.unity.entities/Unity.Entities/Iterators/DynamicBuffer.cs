...
namespace Unity.Entities
{
    ...
    public readonly unsafe struct DynamicBuffer<T> : IQueryTypeParameter, IEnumerable<T>, INativeList<T> where T : unmanaged
    {
...
#if ENABLE_UNITY_COLLECTIONS_CHECKS
                BufferHeader.SetCapacity(m_Buffer, value, UnsafeUtility.SizeOf<T>(), UnsafeUtility.AlignOf<T>(), BufferHeader.TrashMode.RetainOldData, m_useMemoryInitPattern == 1, m_memoryInitPattern, 0);
#else
                BufferHeader.SetCapacity(m_Buffer, value, UnsafeUtility.SizeOf<T>(), UnsafeUtility.AlignOf<T>(), BufferHeader.TrashMode.RetainOldData, false, 0, 0);
#endif
            }
        }
        ...
        public void TrimExcess()
        {
            ...
            m_Buffer->Capacity = Math.Max(length, 0);
...
#if ENABLE_UNITY_COLLECTIONS_CHECKS
            return new DynamicBuffer<U>(m_Buffer, m_Safety0, m_Safety1, m_IsReadOnly == 1, m_useMemoryInitPattern == 1, m_memoryInitPattern, 0);
#else
            return new DynamicBuffer<U>(m_Buffer, 0);
#endif
        }
        ...
        public NativeArray<T> AsNativeArray()
        {
            ...
            var ptr = BufferHeader.GetElementPointer(m_Buffer);
            if (ptr == null)
            {
                ptr = (byte*)m_Buffer;
            }

            var shadow = NativeArrayUnsafeUtility.ConvertExistingDataToNativeArray<T>(ptr, Length, Allocator.None);
#if ENABLE_UNITY_COLLECTIONS_CHECKS
