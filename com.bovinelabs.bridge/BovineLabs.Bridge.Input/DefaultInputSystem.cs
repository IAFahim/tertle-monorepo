...
namespace BovineLabs.Bridge.Input
{
    ...
    [UpdateAfter(typeof(BridgeReadSystemGroup))]
    ...
    public partial class DefaultInputSystem : SystemBase
    {
        ...
        {
            this.cameraQuery = SystemAPI.QueryBuilder().WithAll<CameraMain, CameraBridge>().Build();
            this.RequireForUpdate(this.cameraQuery);

            this.EntityManager.CreateEntity<InputCommon>("Input Common");
        ...
        protected override void OnDestroy()
        {
            if (this.World.IsEditorWorld())
            {
                Object.DestroyImmediate(this.focus.gameObject);
            }
            else
            {
        ...
        {
            var camera = this.cameraQuery.GetSingleton<CameraBridge>().Value.Value;
